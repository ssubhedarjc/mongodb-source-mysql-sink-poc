...
# Initial counts
prev_mongo_count=$(docker exec mongodb mongosh --host localhost:27017 --quiet --eval "use testdb; db.systemusers.countDocuments();" 2>/dev/null || echo "0")
...
while true; do
    # Get current counts
    current_mongo_count=$(docker exec mongodb mongosh --host localhost:27017 --quiet --eval "use testdb; db.systemusers.countDocuments();" 2>/dev/null || echo "0")
    ...
    if [ "$current_mongo_count" -ne "$prev_mongo_count" ]; then
        local diff=$((current_mongo_count - prev_mongo_count))
        if [ $diff -gt 0 ]; then
            ...
            use testdb;
            var latest = db.systemusers.find().sort({_id: -1}).limit(1).toArray()[0];
            if (latest) {
    ...
    if [ "$current_pg_count" -ne "$prev_pg_count" ]; then
        local diff=$((current_pg_count - prev_pg_count))
        if [ $diff -gt 0 ]; then
    ...
    if [ "$current_mongo_count" -ne "$current_pg_count" ]; then
        local sync_diff=$((current_mongo_count - current_pg_count))
